...
class _ApiConfigSectionState extends ConsumerState<ApiConfigSection> {
  ...
  final _profileModelController = TextEditingController();
  final _profileModelsController = TextEditingController();
  ...
  String _editableSignature(ApiProfile? p) =>
      '${p?.id}::${p?.name}::${p?.apiKey}::${p?.baseUrl}::${p?.defaultModel}::${p?.models.join(',')}::${p?.apiProtocol.name}';
  ...
  void dispose() {
    ...
    _profileModelController.dispose();
    _profileModelsController.dispose();
    super.dispose();
  }
  ...
  void _syncProfileControllers(ApiProfile? profile) {
    if (profile == null) {
      ...
      _profileModelController.text = '';
      _profileModelsController.text = '';
      return;
    }
    _profileNameController.text = profile.name;
    _profileApiKeyController.text = profile.apiKey;
    _profileBaseUrlController.text = profile.baseUrl;
    _profileModelController.text = profile.defaultModel;
    _profileModelsController.text = profile.models.join(', ');
  }
  ...
  void _saveProfile(SettingsNotifier notifier, ApiProfile profile) {
    if (_formKey.currentState?.validate() ?? false) {
      final model = _profileModelController.text.trim();
      final updated = profile.copyWith(
        name: _profileNameController.text.trim(),
        apiKey: _profileApiKeyController.text,
        baseUrl: _profileBaseUrlController.text.trim(),
        defaultModel: model.isEmpty ? profile.defaultModel : model,
        models: _parseModels(_profileModelsController.text),
      );
            ...
            children: [
              ...
              if (profile != null) ...[
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(
                    labelText: 'Base URL',
                    hintText: 'https://jeniya.cn 或 https://maas.qianwenaiapi.com/compatible-mode',
                  ),
                  validator: Validators.validateBaseUrl,
                ),
                const SizedBox(height: 16),
                DropdownButtonFormField<ApiProtocol>(
                  initialValue: profile.apiProtocol,
                  decoration: const InputDecoration(
                    labelText: 'API 协议',
                    helperText: '兼容扩展：图编辑走 JSON image，适配千问等 /compatible-mode',
                  ),
                  ...
                  onChanged: (value) {
                    ...
                    final maxCount = value.isCompatible
                        ? ApiConfig.maxGenerateCountCompat
                        : ApiConfig.maxGenerateCount;
                    final sizes = value.isCompatible
                        ? ApiConfig.imageSizesCompat
                        : ApiConfig.imageSizes;
                    notifier.updateProfile(profile.copyWith(
                      apiProtocol: value,
                      defaultCount:
                          profile.defaultCount.clamp(ApiConfig.minGenerateCount, maxCount),
                      defaultSize: sizes.contains(profile.defaultSize)
                          ? profile.defaultSize
                          : sizes.first,
                    ));
                  },
                ...
                const SizedBox(height: 16),
                TextFormField(
                  controller: _profileModelController,
                  decoration: const InputDecoration(
                    labelText: '默认模型',
                    hintText: '例如：gpt-image-2 / qwen-image-3.0-pro',
                  ),
                  validator: (value) {
                    if (value == null || value.trim().isEmpty) {
                      return '请输入默认模型';
                ...
                const SizedBox(height: 16),
                TextFormField(
                  controller: _profileModelsController,
                  decoration: const InputDecoration(
                    labelText: '可选模型列表（逗号分隔，可空）',
                    hintText: 'qwen-image-3.0-pro, qwen-image-3.0',
                    helperText: '留空则使用内置默认列表；填写后生图/编辑页下拉使用该列表',
                  ),
                  maxLines: 2,
                ),
                const SizedBox(height: 16),
                DropdownButtonFormField<String>(
                  initialValue: (profile.allowedSizes.contains(profile.defaultSize)
                          ? profile.defaultSize
                          : profile.allowedSizes.first),
                  decoration: const InputDecoration(labelText: '默认尺寸'),
                  items: profile.allowedSizes
                ...
                Row(
                  children: [
                    Expanded(child: Text('默认生成数量（最多 ${profile.maxCount}）')),
                    IconButton(
                    ...
                    IconButton(
                      icon: const Icon(Icons.add),
                      onPressed: profile.defaultCount >= profile.maxCount
                          ? null
                          : () {
                              notifier.updateProfile(profile.copyWith(
                                  defaultCount: profile.defaultCount + 1));
